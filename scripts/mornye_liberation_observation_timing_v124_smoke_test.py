...
def main() -> None:
    ...
    boundary.advance_timing_runtime(295 / 60, combat_elapsed=0)
    ...
    boundary.advance_timing_runtime(3 / 60, combat_elapsed=0)
    assert not is_action_valid(swap, boundary.state)[0]
    boundary.advance_timing_runtime(1 / 60, combat_elapsed=0)
    assert is_action_valid(swap, boundary.state)[0]
    assert boundary_instance.ended
    print("mornye_liberation_observation_timing_v124_smoke_test ok hit=277 same=296 swap=300")
