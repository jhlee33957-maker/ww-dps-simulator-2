...
def main() -> None:
    ...
    assert by_id["lynae_visual_impact"]["implementation_status"] == (
        "implemented_v3_spray_paint_scheduled_status_application"
    )
    ...
    assert not any(item["topic"] == "spray_paint_periodic_ticks" for item in unresolved)
    ...
    assert "spray_paint_scheduled_flux_application" in report
