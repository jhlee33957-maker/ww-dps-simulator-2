...
def test_slot_schema_labels_and_metadata() -> None:
    ...
    for slot_index in range(3):
        ...
        assert f"slot_{slot_index}.runtime_all_damage_amp_scaled" in labels
        assert f"slot_{slot_index}.runtime_liberation_damage_amp_scaled" in labels
    for action_slot_index in range(32):
        assert f"action_slot_{action_slot_index}.present" in labels
        assert f"action_slot_{action_slot_index}.available" in labels
        assert f"action_slot_{action_slot_index}.cooldown_remaining_ratio" in labels
...
def test_current_channel_mapping() -> None:
    ...
    action_slot_mapping = env.observation_action_slot_mapping()
    assert slot_mapping == {"slot_0": "mornye", "slot_1": "aemeath", "slot_2": "dummy_sub_dps"}
    assert action_slot_mapping["action_slot_0"] == "mornye_basic_attack"
