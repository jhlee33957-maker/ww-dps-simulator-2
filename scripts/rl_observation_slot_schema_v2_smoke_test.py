...
def test_observation_schema_v3_shape_and_mapping() -> None:
    ...
    assert env.observation_space.shape == EXPECTED_V3_SHAPE
    assert env.observation_space.shape != OLD_V2_SHAPE
    assert env.observation_space.shape != OLD_V1_SHAPE
    assert len(env._get_observation()) == len(labels) == EXPECTED_V3_SHAPE[0]
...
def test_party_shape_stays_stable_with_new_channels() -> None:
    ...
    assert aemeath.observation_space.shape == duo.observation_space.shape == EXPECTED_V3_SHAPE


def main() -> None:
    test_observation_schema_v3_shape_and_mapping()
    test_party_shape_stays_stable_with_new_channels()
    print("rl_observation_slot_schema_v2_smoke_test ok (v3 schema)")
