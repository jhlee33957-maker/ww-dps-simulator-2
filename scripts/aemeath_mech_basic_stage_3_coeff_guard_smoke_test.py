...
EXPECTED = [0.1165, 0.0389, 0.0389, 0.0389, 0.0389, 0.8154]
...
def main() -> None:
    ...
    assert actual.count(0.0389) == 4, f"Expected four 0.0389 hits, got {actual.count(0.0389)}"
    ...
    assert all("source_ref" in hit for hit in action["hits"])
    assert any("repeat_source_note" in hit for hit in action["hits"])
