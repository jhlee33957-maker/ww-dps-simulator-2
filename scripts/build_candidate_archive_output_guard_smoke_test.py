...
def main() -> None:
    with TemporaryDirectory() as temp_dir:
        ...
        with zipfile.ZipFile(output) as zf:
            names = zf.namelist()
        name_set = set(names)
        assert "models/maskable_ppo_bc_v105.zip" in name_set
        assert "models/maskable_ppo_candidate_after_bc_v105.zip" in name_set
        ...
        assert not any(name.endswith(".zip") and name not in {
