...
def main() -> None:
    ...
    assert len(stage_2["hits"]) == 6
    assert all(abs(hit["damage_multiplier"] - 0.1690) < 1e-9 for hit in stage_2["hits"])
    ...
    assert len(stage_3["hits"]) == 5
    assert all(abs(hit["damage_multiplier"] - 0.1310) < 1e-9 for hit in stage_3["hits"])
